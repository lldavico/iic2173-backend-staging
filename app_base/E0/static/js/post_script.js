...
Thread.initiate = function () {
    var that = Thread
    that.inner_width = Popup.getInnerWidth();
    that.inner_height = Popup.getInnerHeight();
    // init popups
    that.popup_generic = {
        id: null,
        e: document.getElementById('popup_generic')
    }
    that.popup_report_box = {
        id: null,
        del: null,
        e: document.getElementById('popup_report_box')
    }
    that.popup_reply_box = {
        id: null,
        e: document.getElementById('popup_reply_box')
    }


    window.addEventListener("orientationchange", Thread.onOrientationChange, false);

    var fc_popup_generic = function () {
        Thread.closePopup(Thread.popup_generic, null, true)
    }
    ...
    var fc_report_popup_call = function () {
        Thread.closePopup(Thread.popup_report_box, null, true)
    }
    ...
    var submit_report_call = function (evnt) {
        evnt.preventDefault();
        Calls.onReportSubmit()
    }
    ...
    var fc_reply_popup_call = function () {
        Thread.closePopup(Thread.popup_reply_box, null, true)
    }
    ...
    // var submit_reply_call = function (evnt) {
    //     evnt.preventDefault();
    //     Calls.onReplySubmit()
    // }
    ...
    //var onupdatecall = function (evnt) {
        //evnt.preventDefault();
        //Calls.threadUpdate()
    //}
    ...
    var post_reply_call = function (evnt) {
        evnt.preventDefault();
        Calls.onReplyPopupShow(null)
    }
...
Thread.onOrientationChange = function () {
    var that = Thread
    var new_width = Popup.getInnerWidth(),
        new_height = Popup.getInnerHeight()
    if (new_width !== that.inner_width || new_height !== that.inner_height) {
        that.inner_width = new_width;
        that.inner_height = new_height;
...
//     var hide_button_call = function () {
//         Calls.togglePostHide(p)
//     }
//     hide_button.onclick = hide_button_call

//     var report_button_call = function () {
//         Calls.onReportPopupShow(p, report_button)
//     }
